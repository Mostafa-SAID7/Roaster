# MERZY — Base44 Dev Environment

## Overview
Pure frontend Angular 18 app (MERZY Specialty Coffee & Bakery marketing site in Alexandria, Egypt). No backend, no database, no external services. All data is mocked in `src/app/core/services/`. No secrets required.
...
## Features
- Interactive menu with coffee & bakery items (add-to-cart)
- Slide-out cart drawer with quantity controls
- Order placement and status tracking (Received → Preparing → Ready)
- All content references MERZY, Alexandria, and the Bibliotheca Alexandrina

## Dev server notes
- Angular 18's esbuild-based dev server accepts all hosts by default when bound to 0.0.0.0, so the preview's external hostname works without extra config.
